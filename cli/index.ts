...
function cmdVerify(args: Args): void {
  const { wp } = loadPuzzle(args);
  const problems = validateStructure(wp.threadCount, wp.constraints);
  ...
  const ok = wellFormed && unique && deducible && roundTrip;
  console.log(`structure:    ${wellFormed ? "PASS (§1.3 well-formed)" : "FAIL — " + problems.join("; ")}`);
...
function cmdBatch(args: Args): void {
  ...
  const gatePass =
    rep.uniquenessPass && rep.deducibilityPass && rep.zeroR5 && rep.acceptancePass && rep.allBandsPopulated && rep.wireRoundTripPass && rep.perfPass;
...
function renderBatchReport(rep: ReturnType<typeof runBatch>, requested: number, seed: number): string {
  ...
  L.push(`- Wall time: **${rep.totalMs}ms** · per-puzzle gen+verify p95: **${rep.p95Ms}ms**`);
  L.push("");
  L.push(`## Gate checklist`);
  L.push("");
  const chk = (b: boolean) => (b ? "✅" : "❌");
  L.push(`- ${chk(rep.uniquenessPass)} 100% uniqueness (independent DPLL re-count)`);
  L.push(`- ${chk(rep.deducibilityPass)} 100% deducibility at target tier + certificate + solution match`);
  L.push(`- ${chk(rep.zeroR5)} 0 R5-requiring puzzles (no accepted puzzle stalled below its ceiling)`);
  L.push(`- ${chk(rep.acceptancePass)} Acceptance ≥5% per band`);
  L.push(`- ${chk(rep.allBandsPopulated)} All four scalar bands populated`);
  L.push(`- ${chk(rep.wireRoundTripPass)} Wire round-trip byte-identical`);
  L.push(`- ${chk(rep.perfPass)} Perf budgets (§GENERATOR 6): batch <60s, per-puzzle p95 <50ms`);
  L.push("");
  L.push(`## Per-band`);
  L.push("");
